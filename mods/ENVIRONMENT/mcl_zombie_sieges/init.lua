...
function find_bed(pos)
	return minetest.find_node_near(pos,32,{"mcl_beds:bed_red_bottom"})
end

function find_village(pos)
	local bed = find_bed(pos)
	if bed and find_villager(bed) then
		return bed
	end
end
	...
	cond_start  = function(self)
		...
		if t < 0.04 and rnd == 1 then
			...
			for _,p in pairs(minetest.get_connected_players()) do
				local village = find_village(p:get_pos())
				if village then
